...
function get_direction_to_target(from_obj, to_obj) {
    // Get the direction from one object to another
    var dx = to_obj.x - from_obj.x;
    var dy = to_obj.y - from_obj.y;
...
function basic_enemy_ai() {
    ...
    if (target_player) {
        ...
        var direction = Dir.DOWN;  // Default direction
        ...
        return { action: "attack", target: target_player, direction: direction };
    } else {
        return { action: "wait", target: noone, direction: Dir.DOWN };
    }
}
