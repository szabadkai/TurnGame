...
function has_clear_line_of_sight(from_obj, to_obj) {
    // Simple line of sight check using collision_line
    // Check if there are any solid objects between player and target
    var collision = collision_line(from_obj.x, from_obj.y, to_obj.x, to_obj.y, all, false, false);
    
    // Return true if no collision found, or collision is the target itself
    return (collision == noone || collision == to_obj || collision == from_obj);
}
