...
// Handle ESC key - close current UI or exit game
if (keyboard_check_pressed(vk_escape)) {
    if (ui_state != "none") {
        // Close current UI
        close_all_ui();
    } else {
        // No UI open, exit game
        game_end();
    }
}
...
if (keyboard_check_pressed(ord("I"))) {
    ...
    if (target_player == noone) {
        if (variable_global_exists("combat_log")) {
            global.combat_log("UI Manager: No players found");
        }
        exit;
    }
    ...
    if (can_increase_ability_score(target_player)) {
        if (variable_global_exists("combat_log")) {
            global.combat_log("UI Manager: Player has unused ASI, showing level-up overlay");
        }
        show_level_up_overlay(target_player);
    } else {
        // Show player details
        if (variable_global_exists("combat_log")) {
            global.combat_log("UI Manager: Showing player details");
        }
