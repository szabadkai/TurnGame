...
// === NUCLEAR ESC HANDLER ===
// Handle ESC for nuclear-created PlayerDetails
if (keyboard_check_pressed(vk_escape)) {
    var pd = instance_find(obj_PlayerDetails, 0);
    if (pd != noone && pd.visible) {
        pd.visible = false;
        global.combat_log("NUCLEAR: Closed PlayerDetails with ESC");
    } else {
        // No UI open, exit game
        global.combat_log("NUCLEAR: No UI open, exiting game");
        game_end();
    }
}
